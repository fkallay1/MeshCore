...
// index 0 = 10 dBm module output ... index 20 = 30 dBm
...
// expected module output (dBm) for a requested power, after clamping
...
/* ---------------------------------------------------------------------------
   ...
   right after reset, before configuration). No difference has shown up on this
   module, but it is worth knowing if something odd turns up.
   --------------------------------------------------------------------------- */
#if defined(RADIOLIB_GODMODE)

// DS Rev 2.1 s22.3.1/s22.3.2 - same values RadioLib keeps in LR2021_registers.h,
// redefined here so this header does not depend on a RadioLib internal include
...
// DS s22.3.2 - magic at 0x800FF8 must read 0x600DB002, version at 0x800FFC
...
/* Write the image at 0x801000 in 32-word blocks, then activate (opcode 0x012D).
   Skips the whole thing if the patch is already active, so that a future
   RadioLib loading the PRAM itself does not get a second copy written over it. */
template <class T> static inline int16_t nicerf_lora2021f33_pram_load(T& radio) {
  bool already = false;
  nicerf_lora2021f33_pram_status(radio, &already, NULL);
  if (already) return RADIOLIB_ERR_NONE;
...
template <class T> static inline int16_t nicerf_lora2021f33_set_simo(T& radio, bool on) {
  radio.standby();
  // ramp times kept at the RadioLib default resolution; only simo_usage changes
...
#ifndef NICERF_LORA2021F33_STOCK_PA_TABLE
  // what a requested dBm actually becomes: register value (= chip drive) and
  // the module output the datasheet predicts for it. RadioLib refuses any
  // request outside -9..+22 outright, so 22 is always the top of the dial.
