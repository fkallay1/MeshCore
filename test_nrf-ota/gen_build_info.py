#!/usr/bin/env python3
"""
gen_build_info.py — PlatformIO PRE-build skript pre OTA test repeatera.

Pri každom builde:
  - inkrementuje build counter v test_nrf-ota/build_number.txt
  - vygeneruje test_nrf-ota/build_info.h s FW_BUILD_NUMBER + FW_BUILD_TIME

MyMesh (pod -DWITH_LORA_OTA) vypíše "build #N" v heartbeate → po OTA flash z
neho priamo vidíš, ktorý build beží (NEW musí mať vyššie číslo ako OLD). Toto je
zároveň mechanizmus ktorý zaručí OLD != NEW (build# je súčasť kódu → každý build
je iný → patch nie je prázdny).

Zapojené v OTA env (variants/promicro/platformio.ini):
    extra_scripts = pre:test_nrf-ota/gen_build_info.py
    build_flags  += -I test_nrf-ota

DOČASNÉ testovacie lešenie — pre produkčný OTA build to nie je potrebné.
"""
import datetime
from pathlib import Path

_env = None
try:
    Import("env")              # type: ignore  # PlatformIO SCons kontext
    _env = env                 # type: ignore
    PROJ = Path(env["PROJECT_DIR"])   # type: ignore
except Exception:
    PROJ = Path(__file__).resolve().parent.parent

COUNTER = PROJ / "test_nrf-ota" / "build_number.txt"
HEADER  = PROJ / "test_nrf-ota" / "build_info.h"

# SCons načíta extra_scripts viackrát za build — inkrementuj LEN raz za beh,
# inak by sa build_info menil počas kompilácie a app flash by nezodpovedal patchu.
if _env is not None and _env.get("_BUILD_INFO_DONE"):
    pass
else:
    if _env is not None:
        _env["_BUILD_INFO_DONE"] = True

    n = 0
    if COUNTER.exists():
        try:
            n = int(COUNTER.read_text().strip())
        except ValueError:
            n = 0
    n += 1
    COUNTER.write_text(str(n))

    ts = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    HEADER.write_text(
        "// AUTO-GENERATED by test_nrf-ota/gen_build_info.py — NEUPRAVUJ, NEVERZIONUJ\n"
        "#pragma once\n"
        f"#define FW_BUILD_NUMBER {n}\n"
        f'#define FW_BUILD_TIME   "{ts}"\n',
        encoding="utf-8",
    )
    print(f"[build_info] build #{n} @ {ts}  -> {HEADER.name}")
