...
from ota_sender import (make_patch, build_meta_payload, build_sig_payload,
                        ...
                        OTA_MAGIC, OTA_CHUNK_DATA, OTA_CHANNEL_NAME)
...
async def run_mcpy(args):
    ...
    print(f"[mcpy] {len(patch)}B -> {total} chunkov x {OTA_CHUNK_DATA}B; scope={args.scope} path_len={path_len}")
    if not privkey:
        print("[mcpy] WARNING: --privkey nezadaný — podpis nulový (repeater odmietne, ak nemá OTA_ALLOW_UNSIGNED)")
