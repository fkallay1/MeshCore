...
Výstup:
    nrffota/flasher_code.h — static const uint8_t flasher_code[] s ARM Thumb2 kódom
...
IS_V7         = BOARD_FLASHER == "xiao"
BOARD_DEFINE  = "BOARD_XIAO" if IS_V7 else "BOARD_PROMICRO"
APP_BASE_STR  = "0x27000" if IS_V7 else "0x26000"
FLASHER_LD    = NRFOTA_DIR / "flasher" / "flasher.ld"
# Per-board výstup: v6 (s140 v6, app@0x26000) vs v7 (s140 v7, app@0x27000).
# Flasher je standalone (bez app linker symbolov) → app base MUSÍ byť compile-time
# konštanta, preto pre každý SoftDevice samostatný blob. Výber vo FW: FotaPatcher.cpp
# cez #if defined(FOTA_SOFTDEVICE_V7).
OUT_HEADER    = NRFOTA_DIR / ("flasher_code_v7.h" if IS_V7 else "flasher_code_v6.h")
...
def main():
    ...
    lines = [
        ...
        f" * Board: {BOARD_FLASHER} (-D{BOARD_DEFINE}), app base = {APP_BASE_STR}.",
        " * Regenerate after modifying nrffota/flasher/flasher.c:",
        f" *   BOARD_FLASHER={BOARD_FLASHER} python examples/simple_repeater/nrffota/tools/build_flasher.py",
        " */",
        "#pragma once",
        "#include <stdint.h>",
        "",
