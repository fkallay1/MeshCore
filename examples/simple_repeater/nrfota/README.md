# nrfota — LoRa-OTA (delta-patch) pre MeshCore repeater (nRF52840)
...
```
PC (ota_sender.py)                     Repeater (nRF52840)
...
- **Transport:** OTA pakety idú ako MeshCore `PAYLOAD_TYPE_GRP_DATA` na dedikovanom
  ...
  HMAC-SHA256) a `MyMesh::onGroupDataRecv()` ich odovzdá do `ota_process()`.
...
OTA je zapnuté build-flagom `-D WITH_LORA_OTA=1` (viď `variants/promicro/platformio.ini`).
Bez tohto flagu sú všetky `nrfota/` súbory inertné → stock repeater builды sú
nedotknuté. PSK kanála: `-D OTA_CHANNEL_PSK='"..."'` (musí sa zhodovať so senderom).
